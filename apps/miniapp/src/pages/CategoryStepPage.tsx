    ...
    <div className="page">
      ...
      {feed.items.length > 0 ? (
        ...
        !feed.loading && (
          <EmptyState
            icon={isService ? '🛠' : '📦'}
            title={isService ? 'Станьте первым мастером здесь' : 'Здесь пока пусто — будьте первым'}
            hint={
              isService
                ? 'Все, кто зайдёт в этот раздел, увидят только вас. Первая неделя показа анкеты — в подарок.'
                : mode === 'buy'
                  ? 'Напишите, что ищете, — продавцы увидят запрос и откликнутся. Это бесплатно.'
                  : isCareer
                    ? 'Первое объявление в разделе видят все, кто сюда заходит.'
                    : 'Сфотографируйте вещь — объявление по фото составит ИИ, за минуту и бесплатно.'
            }
            action={
              isService
                ? { label: 'Разместить анкету бесплатно', to: '/profile/application' }
                : mode === 'buy'
                  ? { label: 'Создать запрос', to: '/wanted/new' }
                  : mode === 'job'
                    ? { label: 'Разместить вакансию', to: '/career/new-job' }
                    : mode === 'resume'
                      ? { label: 'Разместить резюме', to: '/career/new-resume' }
                      : { label: 'Выложить вещь', to: '/market/sell' }
            }
          />
        )
      )}
