...
bot.command('start', async (ctx) => {
  ...
  /*
   * Пришёл по чужой ссылке-приглашению.
   *
   * Сам код бот никуда не записывает: приглашение фиксирует сервер при
   * входе в приложение, где видно, кто именно пришёл. Задача бота —
   * донести код до приложения нетронутым, поэтому он идёт параметром
   * запуска в той же кнопке.
   */
  if (payload?.startsWith('ref_')) {
    await ctx.reply(messages.INVITED, {
      parse_mode: 'HTML',
      reply_markup: singleButton('🚀 Открыть NADO', payload),
    });
    return;
  }
  ...
  /*
   * Рассказ о площадке — между приветствием и дверями.
   *
   * Человек уже понял, куда попал, но ещё не выбрал раздел: это
   * единственный момент, когда он готов прочитать, чем здесь лучше.
   * После выбора двери ему будет не до того.
   */
  await replyWithBanner(
    ctx,
    'promo',
    messages.PROMO,
    new InlineKeyboard().webApp('🎁 Моя ссылка для друзей', appUrl('referrals')),
    messages.PROMO,
  );
