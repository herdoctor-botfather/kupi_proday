...
const envSchema = z.object({
  ...
  /**
   * Общий ключ для обращений бота к API.
   *
   * Об оплате Telegram сообщает боту, а выдаёт купленное сервер — значит
   * между ними нужен канал, которому можно верить. Ключ должен совпадать
   * с таким же в конфигурации бота; пока он пустой, подтверждения оплаты
   * не принимаются вовсе — это безопаснее, чем принимать их от кого угодно.
   */
