...
export const confirmPaymentSchema = z.object({
   ...
   * Номер списания в Telegram.
   *
   * Необязателен и длинный. При оплате звёздами Telegram присылает
   * строку в сотни символов, а поле было ограничено сотней с лишним —
   * подтверждение отклонялось целиком, деньги списывались, купленное
   * не выдавалось. Предел оставлен только как защита от мусора.
   */
  telegramChargeId: z.string().trim().max(512).optional().default(''),
