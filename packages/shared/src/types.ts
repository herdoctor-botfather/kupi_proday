...
export interface Category {
  ...
  icon: string;
  kind: CategoryKind;
  /** Сколько активных карточек или объявлений в категории и её подкатегориях. */
  itemCount: number;
  /** Раздел, которому принадлежит подкатегория. null — это сам раздел. */
  parentSlug?: string | null;
  /** Подкатегории раздела. У подкатегории пусто. */
  children?: Category[];
}
...
export interface SpecialistDetail extends SpecialistListItem {
  ...
  canChat: boolean;
  /**
   * Аккаунт владельца анкеты. Нужен, чтобы показать на той же странице
   * его объявления и запросы: мастер, который заодно продаёт плиту, —
   * один человек, и заставлять посетителя искать его второй раз незачем.
   * null — карточку завёл администратор, владельца у неё нет.
   */
  userId: string | null;
  services: Service[];
